...
export default function Signup() {
    ...
    const handleSubmit = (e) => {
        ...
        if (pass1.value !== pass2.value) {
            ...
            const user = {
                ...
                last_name: form.elements.formSurname.value,
            ...
            (async () => {
                try {
                    ...
                    if (fetchResult.ok) {
                        await fetch("/api/users/image/save/" + user.username);
    ...
    return (
        <SSRProvider>
            <fieldset className='rounded' style={{
                            ...
                            <Form onSubmit={(e) => { handleSubmit(e) }} style={{ textAlign: "left" }} id="signup-form">
                                ...
                                <Form.Group className="mb-2 " controlId="formUsername">
                                    ...
                                    <Form.Control type="text" placeholder="create a username" name='formUsername' minLength={3} required
                                        onBlur={(e) => { checkUsernameExists(e.target.value); e.target.setCustomValidity("") }} />
                                    {usernameExists && <Alert variant="danger">El usuario ya existe</Alert>}
                                </Form.Group>
                                <Form.Group className="mb-2 " controlId="formEmail">
                                    <Form.Label>Email</Form.Label>
                                    <Form.Control type="email" placeholder="your email" name='formEmail' minLength={10} required
                                        onBlur={(e) => { checkEmailExists(e.target.value); e.target.setCustomValidity("") }} />
                                    {emailExists && <Alert variant="danger">El email ya existe</Alert>}
                                ...
                                <ButtonGroup className="mb-2 " aria-label="Basic example">
                                    <button className='buttonItem blue' type="submit">
                                        Submit
                                    </button>
                                </ButtonGroup>
                            </Form>
